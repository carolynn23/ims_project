...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login'])) {
    $email = filter_var($_POST['email-username'], FILTER_SANITIZE_EMAIL);
    $password = $_POST['password'];

    try {
        $stmt = $pdo->prepare("SELECT id, email, password, role, profile_setup FROM Users WHERE email = ?");
        $stmt->execute([$email]);
        ...
        if ($user && password_verify($password, $user['password'])) {
            ...
            if (strtolower($user['role']) === 'student' && !$user['profile_setup']) {
            ...
            <div class="authentication-inner">
                <div class="card px-sm-6 px-0">
                    <div class="card-body">
                        ...
                        <h4 class="mb-1">Welcome to IMS! 👋</h4>
                        <p class="mb-6">Please sign-in to your account and start the adventure</p>
                        <form id="formAuthentication" class="mb-6" method="POST" action="" novalidate>
